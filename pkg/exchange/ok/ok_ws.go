...
func (b *OkexTrader) getSymbolSub() (p OPParam) {
	p.OP = "subscribe"
...
func (b *OkexTrader) parseUserMsg(message []byte) (err error) {
	...
	case "orders":
		...
		for _, v := range orders {
			...
			temp := NewExchangeData(b.Name, EventOrder, o)
			temp.Symbol = v.InstID
			b.datas <- temp
		}
	...
	case "positions":
		...
		for _, v := range pos {
			...
			temp := NewExchangeData(b.Name, EventPosition, t)
			temp.Symbol = v.InstID
			b.datas <- temp
		}
	default:
