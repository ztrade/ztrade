...
func NewBinanceTradeWithSymbol(cfg *viper.Viper, cltName, symbol string) (b *BinanceTrade, err error) {
	...
	b.Name = "binance"
	if cltName == "" {
		err = fmt.Errorf("must input binance client name")
		return
	}
	isDebug := cfg.GetBool(fmt.Sprintf("exchanges.%s.debug", cltName))
	apiKey := cfg.GetString("binance.key")
	apiSecret := cfg.GetString("binance.secret")
...
func NewBinanceTrade(cfg *viper.Viper, cltName string) (b *BinanceTrade, err error) {
	return NewBinanceTradeWithSymbol(cfg, cltName, "XBTUSD")
}

func (b *BinanceTrade) Init(bus *Bus) (err error) {
...
func (b *BinanceTrade) startWS() (err error) {
...
func (b *BinanceTrade) handleData() {
	var o *OrderInfo
	var ok bool
	var posTime int64
	for {
...
func (b *BinanceTrade) SetCloseChan(closeCh chan bool) {
	b.closeCh = closeCh
	return
}
...
func (b *BinanceTrade) onEventCandleParam(e Event) (err error) {
...
func (b *BinanceTrade) onEventOrder(e Event) (err error) {
	var act TradeAction
...
func (b *BinanceTrade) onEventOrderCancelAll(e Event) (err error) {
	b.cancelAllOrder()
	return
}

func (b *BinanceTrade) onEventWatch(e Event) (err error) {
...
func (b *BinanceTrade) cancelAllOrder() {
...
func (b *BinanceTrade) orderRoutine() {
	var err error
	var ret interface{}
...
func (b *BinanceTrade) emitRecentCandles(param CandleParam, recent int) (tLast int64, err error) {
...
func (b *BinanceTrade) emitCandles(param CandleParam) {
	if param.BinSize != "1m" {
		log.Info("BinanceTrade emit candle binsize not 1m:", param)
		return
	}
	nRecent := 60 * 24 * 3
